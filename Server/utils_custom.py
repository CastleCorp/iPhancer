...
def process_command_args(arguments):
    ...
    if phone == "":
        print("\nPlease specify the camera model")
        sys.exit()

    if phone not in ["iphone", "sony", "blackberry"]:
        print("\nPlease specify iphone, sony, or blackberry")
        sys.exit()

    if resolution not in ["orig", "high", "medium", "small", "tiny"]:
        print("\nPlease specify resolution: orig, high, medium, small, or tiny")

    if use_gpu not in ["true", "false"]:
        print("\nPlease specify correctly the GPU usage: true, or false")
        sys.exit()
    ...
    return phone, resolution, use_gpu, photo
